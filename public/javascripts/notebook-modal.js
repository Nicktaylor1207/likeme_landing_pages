$(function(){	
	...
	$('.nb-notebook-img-container').on('click', function(){
		
		/* Find notebook via nbFinder attribute */
		var nbFinder = parseInt($(this).attr('nbFinder'), 10);
		$('#notebook-modal').attr('nbFinder', nbFinder);
...
$(function(){
	...
	(function(){
		...
		photoModal.on('shown', function(){
			...
			window.history.pushState({stateObj: 'create-nb-pm'},'','create-nb-pm');
		});

		photoModal.on('hidden', function(){
			
			if (location.href.indexOf('create-nb-pm') != -1) {
				window.history.back();
			}
...
$(function(){
	...
	var fullModalClose = $('#full-notebook-modal-close');
	var arrowBg = $('#arrow-bg');
	var fullArrowBg = $('#full-arrow-bg');

	$('#fullscreen').on('click', function(){
		pmRightCtn.hide();
		photoModal.addClass('full-notebook-modal');
		...
		photoModal.on('hide', function(e){
			pmRightCtn.show();
			photoModal.removeClass('full-notebook-modal');
...
$(function(){
	...
	$('#notebook-modal').on('shown', function(){
		setTags();
		setComments();
	});
